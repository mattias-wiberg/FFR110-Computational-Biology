...
n = pickle.load(open("n0_3_N_6_Text_17.4_b_10_d_5.p", "rb" ))
dt = 0.01
seconds_to_plot = [7.40, 17.40, 27.40]
...
def plot():
    ...
    #ax[0].plot(count[:,0], -np.log(count[:,1]), label=r"$- \log P(n_{%s})$" % seconds_to_plot[0])
    ax[0].plot(count[:,0], count[:,1], label=r"$-n \log P(n_{%s})$" % seconds_to_plot[0])
    ...
    count[:,1] = count[:,1]/np.sum(count[:,1])
    #ax[1].plot(count[:,0], -np.log(count[:,1]), label=r"$- \log P(n_{%s})$" % seconds_to_plot[1])
    ax[1].plot(count[:,0], count[:,1], label=r"$-n \log P(n_{%s})$" % seconds_to_plot[1])
    #gauss = np.log(stats.norm.pdf(count[:,0], np.mean(count[:,0]), np.std(count[:,0])))
    #diff = -np.log(count[0,1]) - gauss[0]
    ...
    count[:,1] = count[:,1]/np.sum(count[:,1])
    #ax[2].plot(count[:,0], -np.log(count[:,1]), label=r"$- \log P(n_{%s})$" % seconds_to_plot[2])
    ax[2].plot(count[:,0], count[:,1], label=r"$-n \log P(n_{%s})$" % seconds_to_plot[2])
    #gauss = np.log(stats.norm.pdf(count[:,0], np.mean(count[:,0]), np.std(count[:,0])))
    #diff = -np.log(count[0,1]) - gauss[0]
